...
public class BuiltInTester implements Debugger
{
            ...
            Class<?> inputType, Class<?> outputType)
    {
        ...
        //possibleValue is all the values the input could be, so inputType is the data type of possibleValue
        tempList.add(new Output(possibleValue, expectedOutput, inputType, outputType));
...
//TODO Fix this implementation
    /*
    public int expecting(Object[] inputValue, Object[] possibleValue,
            String expectedOutput, String variableID, String functionID,
            Class<?> type)
    {
        ...
        int returnCode = expecting(inputValue, possibleValue, expectedOutput, variableID, functionID, type);
        ...
        for (Output i : expectedOutputs) {
            if (i.getInputType().isArray()) {
                ...
                for (int j = 0; j < Array.getLength(givenValArr); j++)
                    if (!i.getInputType().getEnclosingClass().cast(Array.get(givenValArr, j)).equals(i.getInputType().getEnclosingClass().cast(Array.get(expectedValArr, j)))) {
                        match = false;
                        break;
                    }
                ...
                if (match) {
                    ...
                    inputValue.append("[");
                    for (int j = 0; j < Array.getLength(givenValArr); j++)
                        inputValue.append(i.getInputType().getEnclosingClass()
                    ...
                    if (i.getExpectedOutput().equals(actualOutput)) {
                        logInternal(true, inputValue.toString(),
                                i.getOutputType().cast(actualOutput).toString(), i.getOutputType().cast(i.getExpectedOutput()).toString(),
                ...
                if (match) {
                	...
                	// So instead of comparing strings, I am comparing objects, hence the i.getOutputType().cast(actualOutput).toString method call.
                	// If the actual output and the expected output are the same, then you have a PASS.
                    ...
                    if (i.getExpectedOutput().equals(actualOutput)) {
                        ...
                        logInternal(false, i.getInputType().cast(givenVal)
                                .toString(),  i.getOutputType().cast(actualOutput).toString(),
...
class Output
{
    ...
    public boolean equals(Object other)
    {
        return other == this
                || ((other instanceof Output)
                        && getInputType().cast(((Output) other).possibleInputValue)
                                .equals(getInputType().cast(possibleInputValue)) && ((Output) other).expectedOutput
    ...
    public int hashCode()
    {
        ...
        result = prime * result
                + ((possibleInputValue == null) ? 0 : possibleInputValue.hashCode());
    ...
    {
        return "Output [possibleInputValue=" + possibleInputValue + ", expectedOutput="
                + expectedOutput + ", inputType=" + inputType + "]";
    }
}